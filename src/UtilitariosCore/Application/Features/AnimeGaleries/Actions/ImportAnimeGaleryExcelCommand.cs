...
public record ImportAnimeGaleryExcelCommand : IRequest<Result<ImportExcelResult>>
{
    ...
    : IRequestHandler<ImportAnimeGaleryExcelCommand, Result<ImportExcelResult>>
{
    public async Task<Result<ImportExcelResult>> Handle(ImportAnimeGaleryExcelCommand request, CancellationToken cancellationToken)
    {
        ...
        var data = ExcelHelper.ReadAnimeGaleryExcel(stream);
        ...
        foreach (var row in data.Galeries.OrderBy(r => r.Id <= 0 ? int.MaxValue : r.Id))
        {
            ...
            var existing = row.Id > 0 ? await repository.GetAnimeGaleryById(row.Id) : null;

            if (existing is null)
            {
                existing = await repository.GetAnimeGaleryByName(normalizedName);
            }

            if (existing is null)
            {
                var newId = await repository.CreateAnimeGalery(new AnimeGalery
                {
                    Name = normalizedName,
            ...
            await repository.UpdateAnimeGalery(existing);
            updated++;
        ...
        foreach (var mediaRow in data.Media)
        {
            ...
            var existingMedia = await mediaRepository.GetMediaByRefId(galeryId, MediaType.AnimeGalery);
