...
public record UploadActressAdultImageCommand : IRequest<Result>
{
    public int ActressId { get; init; }
    public byte[] ImageData { get; init; } = [];
    public string FileName { get; init; } = string.Empty;

    public sealed class Validator : AbstractValidator<UploadActressAdultImageCommand>
    {
        public Validator()
        {
...
internal sealed class UploadActressAdultImageCommandHandler(
    IActressAdultRepository actressRepository,
    IImgBBService imgBBService)
    : IRequestHandler<UploadActressAdultImageCommand, Result>
{
    public async Task<Result> Handle(UploadActressAdultImageCommand request, CancellationToken cancellationToken)
    {
        var actress = await actressRepository.GetActressAdultById(request.ActressId);
        if (actress is null)
        ...
        await actressRepository.UpdateActressAdultImage(request.ActressId, uploadResult.Url);
