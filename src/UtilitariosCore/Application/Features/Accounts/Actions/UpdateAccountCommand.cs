...
public record UpdateAccountCommand(
    ...
    List<UpdateAccountPropertyRequest>? Properties,
    List<UpdateAccountRenewalRequest>? Renewals
) : IRequest<Result>;
...
internal sealed class UpdateAccountCommandHandler(IAccountRepository accountRepository)
    : IRequestHandler<UpdateAccountCommand, Result>
{
    public async Task<Result> Handle(UpdateAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        var account = new Account
        {
            ...
            CreatedAt = DateTime.Now // no se actualiza en DB pero se requiere para el model
        };
        ...
        if (!updated) return Errors.BadRequest("No se pudo actualizar la cuenta.");
